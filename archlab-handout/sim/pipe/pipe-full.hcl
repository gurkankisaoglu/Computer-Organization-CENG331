...
# 2171726
#
# Since we want IADDQ as valid instruction, first we add it into instr_valid.
# Our instruction takes an immediate and register, so we add it into need_regids
# and need_valC. Since second operand is reg, we use decoded reg for rB in srcB and the same
# 3 reg is used as destination of the alu. So: d_dstE is D_rB. And pass the constant val
# from execute stahe to inp A of ALU and valB to second input.And finally, we set the condition
# flags.
#
####################################################################
